(function (factory) {
  ...
  Backbone.Paginator = (function ( Backbone, _, $ ) {
    ...
    Paginator.clientPager = Backbone.Collection.extend({
      ...
      filterFields: "",
      filterExpression: "",
      filterArrayExpression: "",
      lastFilterExpression: "",
      lastFilterArrayExpression: "",
...
*/
      ...
      setArrayFilter: function ( filter ) {
        if( filter !== undefined ){
          this.lastFilterArrayExpression = this.filterArrayExpression;
          this.filterArrayExpression = filter;
          this.fetcher();
          // this.info();
        }
      },
      ...
      doFakeArrayFilter: function ( filter ) {
        if( filter !== undefined ){
          ...
          testModels = this._filterArray(testModels, filter);
      ...
      pager: function (options) {
        ...
        self.sorter();
        self.info();
        self.trigger('updateRow',options);
      ...
      // We need to run a fetch first to get new data before running pager - so we do it with a callback
      fetcher: function( options ) {
        var self = this,
        limit = self.perPage * 6;

        self.sorter();
        self.info();

        var info = self.information;
        options = options || {};

        // merge custom data into defaults
        options = _.extend(options, {
          remove: false,
          success: function( response ){ self.pager(); },
          data: { limit: limit }
        });
      ...
      sorter: function() {
        ...
        // orgininal models should always include the new set from the server
        // our origModel set
        else {
          //self.origModels = _.union(self.origModels,self.models);
          //_.uniq(self.origModels, false, function( model ){ return model.id; })
        }
        ...
        // Check if array filtering was set using setArrayFilter.
        if ( this.filterArrayExpression !== "" ) {
          self.models = self._arrayFilter(self.models, this.filterArrayExpression);
        }

        // If the sorting or the filtering was changed go to the first page
        // if ( this.lastSortColumn !== this.sortColumn || this.lastFilterExpression !== this.filterExpression || this.lastFilterArrayExpression !== this.filterArrayExpression || !_.isEqual(this.fieldFilterRules, this.lastFieldFilterRules) ) {


        //console.log(_.difference(this.filterArrayExpression.toJSON(),this.lastFilterArrayExpression.toJSON()));

        // if ( this.lastFilterArrayExpression !== this.filterArrayExpression ) {
        if ( cat.Options.filterChange === true ) {
          ...
          // need something better here to keep the extenion generic
          cat.Options.filterChange = false;
          // this.lastFilterArrayExpression = this.filterArrayExpression;
        }
      ...
      _fieldFilter: function( models, rules ) {
        ...
        _.each(models, function(model){
          ...
          // catly each rule to each model in the collection
      ...
      _arrayFilter: function( models, filter ){

        if( filter === '' || filter === undefined || filter === []) return models;

        var keywords = filter.get('keywords');

        function mapFilterArray( collection ) {
          var selectedSet = filter.get(collection).where({ selected: true });
        ...
        var filteredModels = _.filter(models, function(entry){
          ...
          // cycle viewtime
          if (viewtimeList.length > 0) {

            found = false;
            var viewtimes = _.values(tagSet.viewing_time);
            _.each(viewtimes,function(viewtime){
              if (!found) { found = _.contains(viewtimeList,viewtime); }
            });
            if (!found) return false;
          }
      ...
      info: function () {
        var self = this,
        info = {},
        totalFetchedRecords = (self.sortedAndFilteredModels) ? self.sortedAndFilteredModels.length : self.length,
        totalServerRecords = self.totalRecords;
        //totalPages = Math.ceil(totalRecords / self.perPage);

        info = {
          totalUnfilteredRecords: self.origModels.length,
          totalRecords: totalFetchedRecords,
          totalServerRecords: totalServerRecords || 0,
          currentPage: self.currentPage,
          perPage: self.perPage,
          totalPages: Math.ceil(totalServerRecords / self.perPage),
          renderedPages: Math.ceil(totalFetchedRecords / self.perPage),
          lastPage: Math.ceil(totalServerRecords / self.perPage),
          previous: false,
          next: false,
          startRecord: totalFetchedRecords === 0 ? 0 : (self.currentPage - 1) * this.perPage + 1,
          endRecord: Math.min(totalServerRecords, self.currentPage * this.perPage)
        };
